...
def make_supervisor_kitchen(llm: BaseChatModel, members: list[str]):
    ...
    system_prompt = (
        ...
        "   - Route to analyze_kitchen_node to collect data\n"
        ...
        "DECISION LOGIC:\n"
        "- If analyze_kitchen_node not called yet → route to analyze_kitchen_node\n"
        "- If analyze_kitchen_node completed AND visualization NOT called:\n"
        "  → If visualization would help → route to 'kitchen_visualization_node'\n"
    ...
    def supervisor_node(state: State) -> Command[Literal[*members, "kitchen_visualization_node", "__end__"]]:
        ...
        for msg in state["messages"]:
            if hasattr(msg, 'name') and msg.name == "kitchen_supervisor":
                if "VISUALIZATION: Generating graphs" in msg.content:
                    visualization_called = True
            if hasattr(msg, 'name') and msg.name == "kitchen_visualization":
        ...
        # Safety checks
        if goto == "kitchen_visualization_node" and (visualization_called or visualization_completed):
